    ...
    try {
        ...
        if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
            try {
                ...
                console.error('[digest] Failed to parse model output as JSON even after brace-extraction. Raw output:', raw);
                return null;
            }
        }
        console.error('[digest] Failed to parse model output as JSON, and no {...} block found. Raw output:', raw);
        return null;
    }
}
